          ...
          <nav className="hidden md:flex items-center gap-8 text-lg">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className={`transition duration-200 ${pathname === link.href ? "text-pink-600 font-semibold" : "text-gray-700 hover:text-pink-600"}`}
              >
                {link.name}
              </Link>
            ))}
      ...
      <div className={`md:hidden transition-all duration-300 overflow-hidden ${mobileOpen ? "max-h-96" : "max-h-0"}`}>
        <div className="px-6 pb-6 flex flex-col gap-4 bg-white">
          ...
          <Link href="/cart" onClick={() => setMobileOpen(false)} className="relative">
            Cart
            {totalCount > 0 && (
              <span className="absolute -top-2 -right-0 text-xs bg-pink-600 text-white px-1.5 rounded-full">
                {totalCount}
              </span>
            )}
          </Link>
